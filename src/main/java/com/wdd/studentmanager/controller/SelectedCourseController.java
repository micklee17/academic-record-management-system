...
public class SelectedCourseController {
    ...
    public AjaxResult addSelectedCourse(SelectedCourse selectedCourse){
        ...
        try {
            ...
            if(count == 1){
                ...
                ajaxResult.setMessage("Course enrollment successful.");
            }else if(count == 0){
                ajaxResult.setSuccess(false);
                ajaxResult.setMessage("Course enrollment full.");
            }else if(count == 2){
                ajaxResult.setSuccess(false);
                ajaxResult.setMessage("Course already selected.");
            }
        }catch (Exception e){
            e.printStackTrace();
            ajaxResult.setSuccess(false);
            ajaxResult.setMessage("Internal system error, please contact the administrator.");
        }
        return ajaxResult;
    }
    ...
    public AjaxResult deleteSelectedCourse(Integer id){
        ...
        try {
            ...
            if(count > 0){
                ajaxResult.setSuccess(true);
                ajaxResult.setMessage("Removal successful.");
            }else{
                ajaxResult.setSuccess(false);
                ajaxResult.setMessage("Removal failed.");
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
