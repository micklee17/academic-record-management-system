...
public class SystemController {
    ...
    public AjaxResult submitlogin(String username, String password, String code, String type,
        ...
        if(StringUtils.isEmpty(username)){
            ...
            ajaxResult.setMessage("Please enter your username.");
            return ajaxResult;
        }
        if(StringUtils.isEmpty(password)){
            ajaxResult.setSuccess(false);
            ajaxResult.setMessage("Please enter your password.");
            return ajaxResult;
        }
        if(StringUtils.isEmpty(code)){
            ajaxResult.setSuccess(false);
            ajaxResult.setMessage("Please enter the verification code.");
            return ajaxResult;
        }
        if(StringUtils.isEmpty(session.getAttribute(Const.CODE))){
            ajaxResult.setSuccess(false);
            ajaxResult.setMessage("Session timeout, please refresh.");
            return ajaxResult;
        }else{
            if(!code.equalsIgnoreCase((String) session.getAttribute(Const.CODE))){
                ajaxResult.setSuccess(false);
                ajaxResult.setMessage("Incorrect verification code.");
                return ajaxResult;
            }
        }
        //数据库校验
        switch (type){
            case "1":{ //管理员
                ...
                if(StringUtils.isEmpty(ad)){
                    ajaxResult.setSuccess(false);
                    ajaxResult.setMessage("Incorrect username or password.");
                    return ajaxResult;
                }
    ...
    public AjaxResult editPassword(String password,String newpassword,HttpSession session){
        ...
        if (usertype.equals("1")){
            ...
            if(!password.equals(admin.getPassword())){
                ajaxResult.setSuccess(false);
                ajaxResult.setMessage("Incorrect original password.");
                return ajaxResult;
            }
            ...
            try{
                ...
                if(count > 0){
                    ajaxResult.setSuccess(true);
                    ajaxResult.setMessage("Modification successful. Please log in again.");
                }else{
                    ajaxResult.setSuccess(false);
                    ajaxResult.setMessage("Modification failed.");
                }
            }catch (Exception e){
                e.printStackTrace();
                ajaxResult.setSuccess(false);
