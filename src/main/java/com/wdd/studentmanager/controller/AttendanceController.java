...
public class AttendanceController {
    ...
    public AjaxResult addAttendance(Attendance attendance){
        ...
        if(attendanceService.isAttendance(attendance)){
            ...
            ajaxResult.setMessage("Duplicate check-in detected! Please refrain from signing in again!！");
        }else{
            int count = attendanceService.addAtendance(attendance);
            if(count > 0){
                //签到成功
                ajaxResult.setSuccess(true);
                ajaxResult.setMessage("Check-in successful!");
            }else{
                ajaxResult.setSuccess(false);
    ...
    public AjaxResult deleteAttendance(Integer id){
        ...
        try {
            ...
            if(count > 0){
                ajaxResult.setSuccess(true);
                ajaxResult.setMessage("Deletion successful.");
            }else{
                ajaxResult.setSuccess(false);
                ajaxResult.setMessage("Deletion failed.");
            }
        } catch (Exception e) {
            ajaxResult.setSuccess(false);
            ajaxResult.setMessage("System exception, please retry.");
            e.printStackTrace();
        }
        return ajaxResult;
    }
